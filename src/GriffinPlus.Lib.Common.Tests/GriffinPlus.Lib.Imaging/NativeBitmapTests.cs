...
public unsafe class NativeBitmapTests
{
		...
		get
		{
			...
			foreach (PixelFormat format in formats)
			{
				...
				for (int i = 0; i < widthCount; i++)
				{
					...
					CalculateBufferAlignmentAndStride(format, width, out int _, out nint stride);
		...
		BitmapPalette palette)
	{
		...
		fixed (byte* pOriginalBuffer = &originalBuffer[0])
		{
			...
			Assert.Equal((nint)pOriginalBuffer, original.UnsafeBufferStart);

			// create a copy of the bitmap
			using var copy = new NativeBitmap(original);
			// calculate the expected alignment and stride of the backing buffer
			CalculateBufferAlignmentAndStride(format, width, out int alignment, out nint stride);
			...
			Assert.NotEqual(0, copy.UnsafeBufferStart);
			Assert.Equal(0, copy.UnsafeBufferStart & (alignment - 1));
		...
		BitmapPalette palette)
	{
		...
		// between the last pixel of a row and the end of the row as well, the last row may be shorter
		// than the last row of the actual buffer)
		nint originalBufferSize = (height - 1) * originalStride + (width * format.BitsPerPixel + 7) / 8;
		byte[] originalBuffer = new byte[originalBufferSize];
		new Random(0).NextBytes(originalBuffer);
		...
		BitmapPalette palette)
	{
		...
		CalculateBufferAlignmentAndStride(format, width, out int alignment1, out nint bufferStride1);
		...
		CalculateBufferAlignmentAndStride(format, width + 1, out int _, out nint bufferStride2);
	...
	/// <summary>
	/// Tests the <see cref="NativeBitmap(NativeBuffer, int, int, IntPtr, double, double, PixelFormat, BitmapPalette, bool)"/> constructor,
	...
	public void NativeBitmap_FromNativeBuffer_BufferTooSmall()
	{
		...
		CalculateBufferAlignmentAndStride(format, width, out int _, out nint stride);
		nint minimumBufferSize = (height - 1) * stride + (width * format.BitsPerPixel + 7) / 8;
	...
	#region NativeBitmap(nint pImageData, long bufferSize, int width, int height, long stride, double dpiX, double dpiY, PixelFormat format, BitmapPalette palette)

	/// <summary>
	/// Tests the <see cref="NativeBitmap(IntPtr, IntPtr, int, int, IntPtr, double, double, PixelFormat, BitmapPalette)"/> constructor,
	/// getting all properties and disposing the instance.
	/// </summary>
	/// <param name="width">Width of the <see cref="NativeBitmap"/> instance to create.</param>
	/// <param name="height">Height of the <see cref="NativeBitmap"/> instance to create.</param>
		...
		BitmapPalette palette)
	{
		...
		fixed (byte* pBuffer = &buffer[0])
		{
			...
			Assert.Equal(stride, bitmap.BufferStride);
			Assert.Equal(bufferSize, bitmap.BufferSize);
			Assert.Equal((nint)pBuffer, bitmap.UnsafeBufferStart);
		}
	}
	...
	public void NativeBitmap_FromUnmanagedBuffer_WidthTooSmall(int width)
	{
		...
		const double dpi = 100.0;
		PixelFormat format = PixelFormats.Gray8;
		const nint buffer = 0x1; // invalid buffer, but not used anyway
		const nint bufferSize = 1;
		const int stride = 1;
	...
	#region nint UnsafeBufferStart { get; }
	...
	public void BufferStart_Get()
	{
		...
		BitmapPalette palette = BitmapPalettes.WebPalette;
		using var bitmap = new NativeBitmap(width, height, dpiX, dpiY, format, palette);
		Assert.NotEqual(0, bitmap.UnsafeBufferStart);
	}
	...
	public void CopyPixels_Array_BitmapDisposed()
	{
		CopyPixels_RunTestWithError(
			bitmap =>
			{
				CalculateBufferAlignmentAndStride(bitmap.Format, bitmap.PixelWidth, out int _, out nint stride);
	...
	public void CopyPixels_Array_DestinationSizeIsTooSmall(Type bufferElementType)
	{
		...
		CopyPixels_RunTestWithError(
			bitmap =>
			{
				...
				CalculateBufferAlignmentAndStride(bitmap.Format, selectionWidth, out int _, out nint destinationStride);
	...
	#region void CopyPixels(nint destination, long destinationSize, long destinationStride, long sourceRectX, long sourceRectY, long sourceRectWidth, long sourceRectHeight)
		...
		int           selectionHeight)
	{
		...
		// generate a buffer with random data backing the bitmap
		nint bitmapBufferSize = height * stride;
		byte[] buffer = new byte[bitmapBufferSize];
		new Random(0).NextBytes(buffer);
		...
		fixed (byte* pBuffer = &buffer[0])
		{
			...
			void CreateDestinationArrayAndInvokeCopyPixels(int destinationBufferOffset, nint destinationStride)
			{
				// create a new buffer
				nint destinationBufferSize = (height - 1) * destinationStride + (width * format.BitsPerPixel + 7) / 8 + destinationBufferOffset;
				byte[] destinationBuffer = new byte[destinationBufferSize];
				...
				try
				{
					...
					bitmap.CopyPixels(
						(nint)pDestinationBufferAtOffset,
	...
	private static void CopyPixels_RunTestWithError(Action<NativeBitmap> action)
	{
		...
		fixed (byte* pBuffer = &buffer[0])
		{
			using var bitmap = new NativeBitmap((nint)pBuffer, buffer.Length, width, height, stride, dpiX, dpiY, format, palette);
			action(bitmap);
		}
	}
