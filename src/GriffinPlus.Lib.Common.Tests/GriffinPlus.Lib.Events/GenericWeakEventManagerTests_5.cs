...
/// <summary>
/// Unit tests targeting the <see cref="GenericWeakEventManager{TArg1,TArg2,TArg3,TArg4,TArg5}"/> class.
/// </summary>
[Collection(nameof(NoParallelizationCollection))]
public class GenericWeakEventManagerTests_5 : IDisposable
{
	...
	/// <summary>
	/// Initializes an instance the <see cref="GenericWeakEventManagerTests_5"/> class performing common initialization before running a test.
	/// </summary>
	public GenericWeakEventManagerTests_5()
	{
	...
	public void Complete_WithoutSynchronizationContext(bool scheduleAlways)
	{
		...
		int regCount = GenericWeakEventManager<string, string, string, string, string>.RegisterEventHandler(
			this,
			EventName,
			recipient.Handler,
			null,
		...
		Assert.True(GenericWeakEventManager<string, string, string, string, string>.IsHandlerRegistered(this, EventName));
		Assert.True(GenericWeakEventManager<string, string, string, string, string>.IsHandlerRegistered(this, EventName, recipient.Handler));

		// fire event
		GenericWeakEventManager<string, string, string, string, string>.FireEvent(
			this,
			EventName,
			testData1,
			testData2,
		...
		regCount = GenericWeakEventManager<string, string, string, string, string>.UnregisterEventHandler(this, EventName, recipient.Handler);
		Assert.Equal(0, regCount);

		// check whether the handler is not registered anymore
		Assert.False(GenericWeakEventManager<string, string, string, string, string>.IsHandlerRegistered(this, EventName));
		Assert.False(GenericWeakEventManager<string, string, string, string, string>.IsHandlerRegistered(this, EventName, recipient.Handler));
	}
	...
	public void Complete_WithoutSynchronizationContext_FireImmediately(bool scheduleAlways)
	{
		...
		if (scheduleAlways)
		{
			...
			regCount = GenericWeakEventManager<string, string, string, string, string>.RegisterEventHandler(
				this,
				EventName,
				recipient.Handler,
				null,
	...
	public async Task Complete_WithSynchronizationContext(bool scheduleAlways, bool fireOnSameThread)
	{
		...
		await mThread.Factory.Run(
			() =>
			{
				...
				int regCount1 = GenericWeakEventManager<string, string, string, string, string>.RegisterEventHandler(
					this,
					EventName,
					recipient.Handler,
		...
		// unregister event handler
		int regCount2 = GenericWeakEventManager<string, string, string, string, string>.UnregisterEventHandler(this, EventName, recipient.Handler);
	...
	public void GetEventCallers_WithoutSynchronizationContext(bool scheduleAlways)
	{
		...
		GenericWeakEventManager<string, string, string, string, string>.RegisterEventHandler(this, EventName, recipient1.Handler, null, scheduleAlways);
		GenericWeakEventManager<string, string, string, string, string>.RegisterEventHandler(this, EventName, recipient2.Handler, null, scheduleAlways);

		// get event callers
		Action<string, string, string, string, string> callers = GenericWeakEventManager<string, string, string, string, string>.GetEventCallers(this, EventName);
		Assert.NotNull(callers);
		Action<string, string, string, string, string>[] delegates = callers.GetInvocationList().Cast<Action<string, string, string, string, string>>().ToArray();
		Assert.Equal(2, delegates.Length);

		// call handlers
		if (scheduleAlways)
		{
			delegates[0](testData11, testData12, testData13, testData14, testData15);
			delegates[1](testData21, testData22, testData23, testData24, testData25);
	...
	public async Task GetEventCallers_WithSynchronizationContext(bool scheduleAlways, bool fireOnSameThread)
	{
		...
		await mThread.Factory.Run(
			() =>
			{
				...
				GenericWeakEventManager<string, string, string, string, string>.RegisterEventHandler(
					this,
					EventName,
					recipient1.Handler,
					SynchronizationContext.Current,
