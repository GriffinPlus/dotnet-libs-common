...
public class DataSizeTests
{
	...
	public static IEnumerable<object[]> FormattingTestData
	{
		get
		{
			...
			static long MV(int order) // metric value
			{
				long value = 1;
			...
			// ReSharper disable once InconsistentNaming
			static long MTP(int order) // metric turning point
			{
				long value = 1;
			...
			// ReSharper disable once InconsistentNaming
			static long BV(int order) // binary value
			{
				return 1L << (10 * order);
			}

			// ReSharper disable once InconsistentNaming
			static long BTP(int order) // binary turning point
			{
	...
	public static IEnumerable<object[]> FormattingTestData_AutoBase10_Short_Only =>
		from data in FormattingTestData
		let unit = (DataSizeUnit)data[2]
		let style = (DataSizeUnitStyle)data[3]
		where unit == DataSizeUnit.AutoBase10 && style == DataSizeUnitStyle.Short
		select (object[])
		[
