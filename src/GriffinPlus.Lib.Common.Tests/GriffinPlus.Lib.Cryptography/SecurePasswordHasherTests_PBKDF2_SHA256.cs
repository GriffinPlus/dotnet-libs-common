...
namespace GriffinPlus.Lib.Cryptography
{
	...
	/// <summary>
	/// Tests targeting the <see cref="SecurePasswordHasher_PBKDF2_SHA256"/> class.
	/// </summary>
	public class SecurePasswordHasherTests_PBKDF2_SHA256 : SecurePasswordHasherTests_Base
	{
		/// <summary>
		/// Gets the <see cref="SecurePasswordHasher_PBKDF2_SHA256"/> instance to test.
		/// </summary>
		/// <returns></returns>
		protected override SecurePasswordHasher GetHasher() => SecurePasswordHasher.PBKDF2_SHA256;
		...
		/// </summary>
		protected override Regex PasswordHashRegex { get; } = new Regex(@"^\$PBKDF2-SHA256\$(?<iterations>\d+)\$(?<hash>[a-zA-Z0-9+/=]{64})$", RegexOptions.Compiled);
		...
		public static IEnumerable<object[]> TestData_Verify_TooLessFields
		{
			get
			{
				...
				yield return new object[] { "$" };
				yield return new object[] { "$PBKDF2-SHA256$" };
				yield return new object[] { "$PBKDF2-SHA256$10000" };
			}
		}
		...
		{
			get { yield return new object[] { "$PBKDF2-SHA256$x$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=" }; }
		}
		...
		public static IEnumerable<object[]> TestData_Verify_HashSizeTooShortOrToLong
		{
			get
			{
				yield return new object[] { "$PBKDF2-SHA256$10000$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA==" };
				yield return new object[] { "$PBKDF2-SHA256$10000$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" };
			}
		}
		...
		{
			get { yield return new object[] { "$PBKDF2-SHA256$10000$============================================================================================================" }; }
		}

		#endregion

		#region bool SecurePasswordHasher_PBKDF2_SHA256.Verify(string password, string passwordHash)

		/// <summary>
		/// Tests the <see cref="SecurePasswordHasher_PBKDF2_SHA256.Verify(string,string)"/> method.
		...
		#region bool SecurePasswordHasher_PBKDF2_SHA256.Verify(ReadOnlySpan<char> password, ReadOnlySpan<char> passwordHash)

		/// <summary>
		/// Tests the <see cref="SecurePasswordHasher_PBKDF2_SHA256.Verify(ReadOnlySpan{char},ReadOnlySpan{char})"/> method.
		/// The method should throw a <see cref="FormatException"/> if the password hash contains too less fields.
		/// </summary>
		[Theory]
		[MemberData(nameof(TestData_Verify_TooLessFields))]
