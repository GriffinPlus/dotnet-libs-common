...
public class SerialTaskQueueTests
{
	...
	{
		var queue = new SerialTaskQueue();
		var exception = await Assert.ThrowsAsync<ArgumentNullException>(() => queue.Enqueue((Action)null));
	...
	{
		var queue = new SerialTaskQueue();
		var exception = await Assert.ThrowsAsync<ArgumentNullException>(() => queue.Enqueue(null));
