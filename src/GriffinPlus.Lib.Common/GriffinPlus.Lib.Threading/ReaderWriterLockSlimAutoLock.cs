...
public struct ReaderWriterLockSlimAutoLock : IDisposable
{
	...
	public ReaderWriterLockSlimAutoLock(ReaderWriterLockSlim @lock, ReaderWriterLockSlimAcquireKind acquireKind, int timeout)
	{
		...
		mIsLockAcquired = AcquireKind switch
		{
			ReaderWriterLockSlimAcquireKind.Read            => Lock.TryEnterReadLock(timeout),
			ReaderWriterLockSlimAcquireKind.UpgradeableRead => Lock.TryEnterUpgradeableReadLock(timeout),
			ReaderWriterLockSlimAcquireKind.ReadWrite       => Lock.TryEnterWriteLock(timeout),
			var _                                           => throw new ArgumentException("Invalid acquire type.", nameof(acquireKind))
		};
