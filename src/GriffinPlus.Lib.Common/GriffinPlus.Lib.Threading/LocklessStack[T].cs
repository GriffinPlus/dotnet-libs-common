...
public class LocklessStack<T>
{
	...
	public bool TryPush(T element)
	{
		...
		while (true)
		{
			...
			if (item != null)
			{
				...
				if (Interlocked.CompareExchange(ref mFreeStack, nextItem, item) != item) continue;
				item.NextItem = null;
		...
		while (true)
		{
			...
			if (Interlocked.CompareExchange(ref mUsedStack, item, firstItem) != firstItem) continue;
	...
	{
		return TryPushMany(elements, out bool _);
	}
	...
	public bool TryPushMany(T[] elements, out bool first)
	{
		...
		while (true)
		{
			...
			if (Interlocked.CompareExchange(ref mUsedStack, chainStart, firstItem) != firstItem) continue;
	...
	public bool TryPop(out T element)
	{
		...
		while (true)
		{
			...
			Item nextItem = Interlocked.CompareExchange(ref item.NextItem, null, null);
			if (Interlocked.CompareExchange(ref mUsedStack, nextItem, item) != item) continue;
			item.NextItem = null;
		...
		while (true)
		{
			...
			if (Interlocked.CompareExchange(ref mFreeStack, item, firstItem) != firstItem) continue;
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (!mCanGrow)
			{
				...
				while (true)
				{
					...
					if (Interlocked.CompareExchange(ref mFreeStack, chainStart, firstItem) != firstItem) continue;
