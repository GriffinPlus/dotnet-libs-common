...
namespace GriffinPlus.Lib.Imaging
{
	...
	public unsafe class NativeBitmap : IDisposable
	{
		...
		private readonly long          mBufferStride;
			...
			BitmapPalette palette)
		{
			...
			fixed (byte* pSourceArray = &source[0])
			{
				...
				else
				{
					...
					byte* pDestination = (byte*)mBufferStart.ToPointer();
		...
		/// </summary>
		/// <exception cref="ObjectDisposedException">The instance has been disposed.</exception>
		public IntPtr UnsafeBufferStart
		{
			get
			{
				if (mDisposed) throw new ObjectDisposedException(nameof(NativeBitmap));
			...
			int   sourceRectHeight)
		{
			...
			if (destination is byte[] bytes) // handles sbyte[] as well as the runtime does not differentiate between sbyte[] and byte[]
			{
			...
			if (destination is short[] shorts) // handles ushort[] as well as the runtime does not differentiate between short[] and ushort[]
			{
				// determine the usable size of the buffer (in array elements)
			...
			if (destination is int[] ints) // handles uint[] as well as the runtime does not differentiate between int[] and uint[]
			{
				// determine the usable size of the buffer (in array elements)
			...
			if (destination is long[] longs) // handles ulong[] as well as the runtime does not differentiate between long[] and ulong[]
			{
				// determine the usable size of the buffer (in array elements)
			...
			if (destination is float[] floats)
			{
				// determine the usable size of the buffer (in array elements)
			...
			if (destination is double[] doubles)
			{
				// determine the usable size of the buffer (in array elements)
			...
			long   sourceRectHeight)
		{
			...
			if (sourceRectX == 0 && sourceRectY == 0 && sourceRectWidth == mPixelWidth && sourceRectHeight == mPixelHeight)
			{
				...
				if (mBufferStride == destinationStride)
				{
					...
					Buffer.MemoryCopy(mBufferStart.ToPointer(), destination.ToPointer(), bytesToCopy, bytesToCopy);
				}
				else
				{
					// the bitmap and the destination buffer have different strides
					// => copy row by row...
					byte* pSource = (byte*)mBufferStart.ToPointer();
					byte* pSourceEnd = checked(pSource + mPixelHeight * mBufferStride);
					byte* pDestination = (byte*)destination.ToPointer();
			...
			else
			{
				...
				if (sourceRectX * mPixelFormat.BitsPerPixel % 8 == 0)
				{
					...
					byte* pSource = checked((byte*)mBufferStart.ToPointer() + sourceRectY * mBufferStride + sourceRectX * mPixelFormat.BitsPerPixel / 8);
				...
				else
				{
					...
					byte* pSourceRowStart = checked((byte*)mBufferStart.ToPointer() + sourceRectY * mBufferStride + sourceRectX * mPixelFormat.BitsPerPixel / 8);
					byte* pSourceEnd = checked(pSourceRowStart + sourceRectHeight * mBufferStride);
					byte* pDestinationRowStart = (byte*)destination.ToPointer();
		...
		private static long GetBufferStride(int width, PixelFormat format)
		{
			checked
			{
				if (format.BitsPerPixel <= 32) return ((long)width * format.BitsPerPixel + 31) / 32 * 4;
				if (format.BitsPerPixel <= 64) return ((long)width * format.BitsPerPixel + 63) / 64 * 8;
				return ((long)width * format.BitsPerPixel + 127) / 128 * 16;
			}
		}
	}
