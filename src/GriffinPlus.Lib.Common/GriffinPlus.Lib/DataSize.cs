...
public readonly struct DataSize : IComparable<DataSize>, IEquatable<DataSize>, IFormattable
{
	...
	public string ToString(string format, IFormatProvider formatProvider)
	{
		...
		if (modifierIndex >= 0)
		{
			ReadOnlySpan<char> unitSpecifier = remaining[..modifierIndex].Trim();
			remaining = remaining[(modifierIndex + 1)..];
		...
		DataSizeUnitStyle unitStyle = DataSizeUnitStyle.Long)
	{
		...
		Rule rule;
		if (unit is DataSizeUnit.AutoBase2 or DataSizeUnit.AutoBase10)
		{
			int i = 0;
