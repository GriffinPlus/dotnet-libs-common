...
namespace GriffinPlus.Lib
{
	...
	public static partial class Immutability
	{
		...
		static Immutability()
		{
			...
			AddImmutableType(typeof(IntPtr), true, Reason_PrimitiveType);
			AddImmutableType(typeof(UIntPtr), true, Reason_PrimitiveType);
		...
		/// <returns>Information about the registered type.</returns>
		/// <exception cref="ArgumentNullException">The specified <paramref name="type"/> is <c>null</c>.</exception>
		private static Info AddImmutableType(Type type, bool hasImmutableDerivationsOnly, string reason)
		{
			if (type == null) throw new ArgumentNullException(nameof(type));
		...
		private static bool AreTypeAndDerivedTypesImmutable(IDictionary<Type, Info> cache, Type type)
		{
			...
			if (type.IsVisible)
			{
				...
				// cannot guarantee immutability as we cannot see all derived classes that may exit
				return false;
			}
			...
			// all derived types are immutable
			return true;
		}
	}
