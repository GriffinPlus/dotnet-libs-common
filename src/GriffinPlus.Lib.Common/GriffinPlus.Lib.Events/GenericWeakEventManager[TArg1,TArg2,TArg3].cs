...
/// <summary>
/// Event manager that administrates weak event handlers in a central place (for generic actions using <see cref="Action{TArg1,TArg2,TArg3}"/>).<br/>
...
public static partial class GenericWeakEventManager<TArg1, TArg2, TArg3>
{
		...
		TArg3                       arg3)
	{
		...
		if (context != null)
		{
			if (scheduleAlways) context.Post(_ => handler(arg1, arg2, arg3), null);
		...
		TArg3  arg3)
	{
		...
		foreach (Item item in items)
		{
			if (item.SynchronizationContext != null)
			{
				...
				{
					item.SynchronizationContext.Post(x => { ((Item)x!).Fire(arg1, arg2, arg3); }, item);
				}
			}
			else
			{
	...
	public static Action<TArg1, TArg2, TArg3> GetEventCallers(object obj, string eventName)
	{
		Item[] items;
		...
		Action<TArg1, TArg2, TArg3> handlers = null;
		foreach (Item item in items)
		{
			if (item.SynchronizationContext != null)
			{
