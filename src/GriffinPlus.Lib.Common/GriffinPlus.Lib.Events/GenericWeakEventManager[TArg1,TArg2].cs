...
/// <summary>
/// Event manager that administrates weak event handlers in a central place (for generic actions using <see cref="Action{TArg1,TArg2}"/>).<br/>
...
public static partial class GenericWeakEventManager<TArg1, TArg2>
{
		...
		TArg2                  arg2)
	{
		...
		if (context != null)
		{
			if (scheduleAlways) context.Post(_ => handler(arg1, arg2), null);
			else handler(arg1, arg2);
		}
		else
		{
			if (scheduleAlways) Task.Run(() => handler(arg1, arg2));
			else handler(arg1, arg2);
		}
		...
		TArg2  arg2)
	{
		...
		foreach (Item item in items)
		{
			if (item.SynchronizationContext != null)
			{
				...
				{
					item.SynchronizationContext.Post(x => { ((Item)x!).Fire(arg1, arg2); }, item);
				}
			}
			else
			{
				...
				if (item.ScheduleAlways) Task.Run(() => item.Fire(arg1, arg2));
				else item.Fire(arg1, arg2);
			}
		}
	}
	...
	public static Action<TArg1, TArg2> GetEventCallers(object obj, string eventName)
	{
		Item[] items;
		...
		Action<TArg1, TArg2> handlers = null;
		foreach (Item item in items)
		{
			if (item.SynchronizationContext != null)
			{
			...
			else
			{
				...
				handlers += (arg1, arg2) =>
				{
					if (itemCopy.ScheduleAlways) Task.Run(() => itemCopy.Fire(arg1, arg2));
					else itemCopy.Fire(arg1, arg2);
				};
			}
		}
