...
/// <summary>
/// Event manager that administrates weak event handlers in a central place (for generic actions using <see cref="Action{TArg}"/>).<br/>
...
/// </summary>
/// <typeparam name="TArg">Type of the argument passed to event handlers.</typeparam>
public static partial class GenericWeakEventManager<TArg>
{
	...
	/// </param>
	/// <param name="arg">Argument to pass to the event handler that is fired immediately.</param>
	/// <returns>Total number of registered event handlers (including the specified event handler).</returns>
	public static int RegisterEventHandler(
		object                 obj,
		string                 eventName,
		...
		TArg                   arg)
	{
		...
		if (context != null)
		{
			if (scheduleAlways) context.Post(_ => handler(arg), null);
			else handler(arg);
		}
		else
		{
			if (scheduleAlways) Task.Run(() => handler(arg));
			else handler(arg);
		}
	...
	/// <param name="obj">Object providing the event.</param>
	/// <param name="eventName">Name of the event.</param>
	/// <param name="arg">Event argument to pass to invoked event handlers.</param>
		...
		TArg   arg)
	{
		...
		foreach (Item item in items)
		{
			if (item.SynchronizationContext != null)
			{
				...
				{
					item.SynchronizationContext.Post(x => { ((Item)x!).Fire(arg); }, item);
				}
			}
			else
			{
				...
				if (item.ScheduleAlways) Task.Run(() => item.Fire(arg));
				else item.Fire(arg);
			}
		}
	}
	...
	public static Action<TArg> GetEventCallers(object obj, string eventName)
	{
		Item[] items;
		...
		foreach (Item item in items)
		{
			...
			else
			{
				...
				handlers += arg =>
				{
					if (itemCopy.ScheduleAlways) Task.Run(() => itemCopy.Fire(arg));
					else itemCopy.Fire(arg);
				};
			}
		}
