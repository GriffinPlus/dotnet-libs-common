...
public static partial class GenericWeakEventManager<TArg>
{
		...
		TArg                   arg)
	{
		...
		else
		{
			if (scheduleAlways)
			{
				while (!ThreadPool.QueueUserWorkItem(_ => handler(arg)))
				{
					Thread.Sleep(50);
				}
			}
		...
		TArg   arg)
	{
		...
		foreach (Item item in items)
		{
			...
			else
			{
				...
				{
					while (!ThreadPool.QueueUserWorkItem(_ => item.Fire(arg)))
					{
						Thread.Sleep(50);
					}
				}
		...
		foreach (Item item in items)
		{
			...
			else
			{
				...
				handlers += arg =>
				{
					...
					{
						while (!ThreadPool.QueueUserWorkItem(_ => itemCopy.Fire(arg)))
						{
							Thread.Sleep(50);
						}
					}
