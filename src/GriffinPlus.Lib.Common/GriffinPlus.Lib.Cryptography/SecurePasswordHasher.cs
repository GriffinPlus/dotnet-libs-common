...
#error Unhandled target framework.
	...
	{
		return Hash(password, DefaultIterationCount);
	}
	...
	/// <returns>The password hash.</returns>
	/// <exception cref="ArgumentNullException"><paramref name="password"/> is <c>null</c>.</exception>
	public virtual string Hash(ReadOnlySpan<char> password)
	{
		return Hash(password, DefaultIterationCount);
	}
		...
		int           iterations)
	{
		...
		RandomNumberGenerator.Fill(buffer.AsSpan()[..saltSize]);
#else
#error Unhandled target framework.
#endif
...
#elif NETSTANDARD2_1 || NET5_0 || NET6_0 || NET7_0 || NET8_0
		int encodedPasswordLength = Encoding.UTF8.GetBytes(password, buffer.AsSpan()[encodedPasswordOffset..]);
#else
#error Unhandled target framework.
#endif
		for (int i = 0; i < iterations; i++) hashAlgorithm.TransformBlock(buffer, encodedPasswordOffset, encodedPasswordLength, buffer, encodedPasswordOffset);
...
#elif NETSTANDARD2_1 || NET48 || NET5_0 || NET6_0 || NET7_0 || NET8_0
...
#error Unhandled target framework.
		...
		string        passwordHash)
	{
		...
		saltedHashBytes = saltedHashBytes[..saltedHashByteCount];
#else
#error Unhandled target framework.
#endif
		...
		// get salt
		byte[] salt = saltedHashBytes[..saltSize].ToArray();
...
#error Unhandled target framework.
		...
		// get result
		Debug.Assert(hash != null);
		return saltedHashBytes[saltSize..].SequenceEqual(hash.AsSpan());
	}
...
#error Unhandled target framework.
		...
		out ReadOnlySpan<char> saltedHash)
	{
		...
		if (index < 0) return false;
		algorithm = remaining[..index];
		remaining = remaining[(index + 1)..];

		// ... followed by the number of iterations
		index = remaining.IndexOf('$');
		if (index < 0) return false;
		iterations = remaining[..index];
		remaining = remaining[(index + 1)..];

		// ... followed by the salted hash
		saltedHash = remaining[..];
