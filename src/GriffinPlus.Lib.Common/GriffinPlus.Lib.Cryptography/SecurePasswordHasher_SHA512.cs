...
namespace GriffinPlus.Lib.Cryptography
{
	...
	sealed class SecurePasswordHasher_SHA512 : SecurePasswordHasher
	{
		...
		/// </summary>
		internal const string AlgorithmNameDefinition = "SHA512";
...
#error Unhandled target framework.
			...
			using (var sha512 = System.Security.Cryptography.SHA512.Create())
			{
				...
				for (int i = 0; i < iterations; i++) sha512.TransformBlock(encodedPassword, 0, encodedPassword.Length, encodedPassword, 0);
				sha512.TransformFinalBlock(encodedPassword, 0, 0);
				hash = sha512.Hash;
			}
		...
		public override bool Verify(string password, string hashedPassword)
		{
			...
			if (hashBytes.Length != SaltSize + HashSize)
				throw new FormatException($"The specified hash is not a valid salted SHA512 hash. It must be {SaltSize + HashSize} bytes long.");
