	...
	public class CascadedConfiguration
	{
		...
		public CascadedConfiguration RootConfiguration
		{
			get
			{
				...
				return Parent.RootConfiguration;
			}
		}
		...
		public string GetComment(string path, bool inherit = true)
		{
			...
			lock (Sync)
			{
				...
				// => query the configuration inheriting from (if any)
				return InheritedConfiguration?.GetComment(path, true);
			}
		}
		...
		public ICascadedConfigurationItem[] GetAllItems(bool recursively)
		{
			...
			lock (Sync)
			{
				...
				if (!recursively) return items.ToArray();
